...
public class CropDiseaseService {
    ...
    public CropDisease create(CropDiseaseDto dto){CropDisease x=new CropDisease();x.setCropId(dto.getCropId());x.setDiseaseId(dto.getDiseaseId());x.setDetectedDate(dto.getDetectedDate());x.setSeverity(dto.getSeverity());x.setTreatment(dto.getTreatment());dao.save(x);return x;}
    public void delete(UUID cropId,UUID diseaseId){dao.delete(cropId,diseaseId);}
}
