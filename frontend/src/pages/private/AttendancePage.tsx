...
export default function AttendancePage() {
  ...
  const handleCheckOut = async () => {
    ...
    const result = await attendanceService.checkOut(checkOutTarget.attendanceId);
    toast.success(`Checked out at ${result.checkOut}.`);
    await loadAttendance();
  };
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={
          canManage
            ...
            : "Record your attendance and check out when you're done."
        }
                ...
                >
                  ...
                  <td className="px-5 py-3.5">
                    <RowActions
                      ...
                      onEdit={!canManage && record.checkOut === null ? () => setCheckOutTarget(record) : undefined}
                      editLabel="Check Out"
                      editIcon={faRightFromBracket}
                    />
                  </td>
                </tr>
              ))}
      ...
      {/* Check out of an open attendance record */}
      <ConfirmDialog
        ...
        message={
          checkOutTarget
            ? `Check out now? This will record the current time as your check-out for ${formatDate(checkOutTarget.attendanceDate)}.`
            : ""
        }
